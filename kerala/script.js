...
function selectItineraryDays(days) {
  ...
  // Update itinerary lists
  document.querySelectorAll('.itinerary-list').forEach(el => {
    el.classList.remove('active');
  });
  const itineraryLists = document.querySelectorAll('.itinerary-list');
  if (itineraryLists[idx]) {
    itineraryLists[idx].classList.add('active');
  }

  // Update nav buttons
  updateItineraryIndicator(days);

  // Retrigger reveal animations in new section
  requestAnimationFrame(() => {
    const activeList = document.querySelector('.itinerary-list.active');
    if (activeList) {
      activeList.querySelectorAll('.reveal').forEach(el => {
...
// ===== ITINERARY INDICATOR =====
function updateItineraryIndicator(days) {
  const buttons = document.querySelectorAll('.itinerary-day-btn');
  const indicator = document.getElementById('itineraryIndicator');

  buttons.forEach(btn => {
    btn.classList.toggle('active', parseInt(btn.dataset.days) === days);
  });

  const activeBtn = document.querySelector(`.itinerary-day-btn[data-days="${days}"]`);
  if (activeBtn && indicator) {
    const navInner = document.querySelector('.itinerary-nav-inner');
    if (navInner) {
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => updateItineraryIndicator(3), 200);
});

window.addEventListener('resize', () => updateItineraryIndicator(activeItineraryDays));
...
window.addEventListener('resize', () => {
  initTestimonials();
  updateItineraryIndicator(activeItineraryDays);
});
...
function scrollToSection(id) {
  ...
  if (el) {
    const offset = 72; // navbar height only (removed package switch bar)
...
const sections = ['packages', 'gallery', 'itinerary', 'pricing', 'testimonials'];
